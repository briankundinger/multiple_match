...
fabl_k_result <- matrix(NA, nrow = max_K, ncol = 5)
fabl_penalty_result <- matrix(NA, nrow = max_K, ncol = 5)
multilink_result <- matrix(NA, nrow = max_K, ncol = 5)

for(k in 1:max_K){
start <- proc.time()[3]
out_mm <- fabl_mm(hash, S = S, burn = burn, max_K = k)
time <- proc.time()[3] - start
...
for(k in 1:max_K){
  start <- proc.time()[3]
  out_mm <- fabl_mm(hash, S = S, burn = burn, tau = k - 1)
  time <- proc.time()[3] - start
  result_mm <- estimate_links_mm(out_mm, hash, resolve = F)
  Z_hat <- cbind(result_mm$Z_hat$target_id, result_mm$Z_hat$base_id)
...
saveRDS(result_df, file = paste0("out/sadinle_K_sim/sim_",
                                 str_pad(i, 3, pad = "0")))

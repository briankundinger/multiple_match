...
%		\For{$j\in[n_2]$}{
%			Set $q(Z_j)\propto\exp\{{-Z_j}[\log p(\Omega,\Gamma)] \}$
%		}
%		
%		\For{$f\in[F]$}{
%			Set $q(m_f)\propto\exp\{m_f\}[\log p(\Omega,\Gamma)] \}$
%			
%			Set $q(u_f)\propto\exp\{u_f\}[\log p(\Omega,\Gamma)] \}$
%		}
%		
%		Set $q(\pi) \propto\exp\{E_{-\pi}[\log p(\Omega,\Gamma)] \}$
%		
...
\begin{align}
	\mathbb{P}(\Gamma_{.j}| \bm{m}, \bm{u}, \pi, Z_{j, k}^{(s)} = \emptyset, Z_{j, -k}^{(s)} = q_{-k}) = \prod_{i \notin q_{-k}} \prod_{f=1}^{F}\prod_{l=1}^{L_f} u_{fl}^{I(\gamma_{ij}^f = l)I_{obs}(\gamma_{ij}^f)} = c_{Z_{j, -k}^{(s)}}.
\end{align}

When $Z_{j, k}^{(s)} = q_k$ for some $q_k > 0$, we have
\begin{align}
	\mathbb{P}(\Gamma_{.j}| \bm{m}, \bm{u}, \pi,  Z_{j, k}^{(s)} = q_k, Z_{j, -k}^{(s)} = q_{-k}) &=\prod_{f=1}^{F}\prod_{l=1}^{L_f} m_{fl}^{I(\gamma_{q_k, j}^f = l)I_{obs}(\gamma_{q_k, j}^f)}  \prod_{i \notin (q_{-k}, q_k)}\prod_{f=1}^{F}\prod_{l=1}^{L_f} u_{fl}^{I(\gamma_{ij}^f = l)I_{obs}(\gamma_{ij}^f)} \\

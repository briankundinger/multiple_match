...
%Note that the $k!$ in the final line reflects the fact that there are $k!$ potential orderings of the components $q_c$ that produce the equivalent result $Z_j = q$. Additionally, the two proportionality symbols denote the same normalizing constant, and thus the distribution implied by the sequential sampler is exactly equal to the joint distribution. See Appendix \ref{app:extended-proof} for details. \brian{I believe this is true, but I might need to talk it out to explain better why.}
